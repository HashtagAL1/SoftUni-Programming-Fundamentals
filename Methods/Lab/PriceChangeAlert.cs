...
class PriceChangeAlert
{
    static void Main()
    {
        ...
        for (int i = 0; i < n - 1; i++)
        {
            ...
            double div = Proc(last, c); bool isSignificantDifference = imaliDif(div, threshHold);
    ...
    private static bool imaliDif(double threshHold, double isDiff)
    {
        if (Math.Abs(threshHold) >= isDiff)
        {
            return true;
        }
